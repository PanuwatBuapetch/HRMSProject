...
using HrmsSolution.Service;
    ...
    namespace Hrms_project.Components.Pages.Orignization
    {
        public partial class Organization
        {
            ...
            [Inject]
            public AuthState State { get; set; } = default!;
        ...
        {
            // 1. เช็คว่าล็อกอินหรือยัง จาก State ของคุณเอง
            if (!State.IsLoggedIn)
            {
            ...
            // 2. เช็ค Role จาก State ของคุณเอง 
            isAdmin = State.Role == "SuperAdmin" || State.Role == "Admin" || State.Role == "HR";

            // 3. ถ้าไม่ใช่กลุ่ม Admin ก็ให้เตะกลับหน้าแรก
            if (!isAdmin)
            {
                NavigationManager.NavigateTo("/", true);
                return;
            ...
            // 4. ถ้าผ่านด่านทั้งหมดมาได้ (เป็น Admin) ค่อยโหลดข้อมูลองค์กร
        ...
        {
            searchEmpTerm = e.Value?.ToString() ?? string.Empty;
            HandleEmpSearch();
        }

        private void HandleNewEmpSearchInput(ChangeEventArgs e)
        {
            searchNewEmpTerm = e.Value?.ToString() ?? string.Empty;
